...
const FORMATS: { id: ExportFormat; label: string; desc: string; icon: React.ReactNode }[] = [
  ...
  { id: 'docx', label: 'Word',  desc: 'Editable',   icon: <FileText className="w-6 h-6 text-[var(--text-secondary)]" /> },
...
function integrityLabel(status: IntegrityStatus) {
  if (status === 'verified')        return { text: 'Verified',        cls: 'text-[var(--text-secondary)]' }
...
const missingCount = MOCK_INTEGRITY_ITEMS.filter((i) => i.status === 'missing').length
const reviewCount  = MOCK_INTEGRITY_ITEMS.filter((i) => i.status === 'review_required').length
const totalIssues  = missingCount + reviewCount
  ...
  return (
    <div className="min-h-[calc(100vh-44px)] bg-[var(--bg-secondary)] flex flex-col items-center pt-12 px-4">
      <div className="w-full max-w-xl">
        <h1 className="text-xl font-medium text-center mb-1">Final Review &amp; Export</h1>
        <p className="text-xs text-[var(--text-secondary)] text-center mb-8">
          Please verify all compliance checkpoints before generating the final proposal files.
        </p>

        {/* Integrity check */}
        <Card className="mb-4">
          <div className="flex items-center justify-between mb-4">
            <p className="text-[10px] font-medium text-[var(--text-tertiary)] tracking-wider uppercase">Proposal Integrity Check</p>
            <span className={cn(
              'text-xs font-medium px-2 py-0.5 rounded',
              complete >= 90 ? 'bg-success-50 text-success-600' : 'bg-[var(--bg-secondary)] text-[var(--text-secondary)]'
            )}>
              {complete}% Complete
            </span>
          </div>
          ...
          {totalIssues > 0 && (
            <div className="mt-4 px-3 py-2.5 bg-[var(--warning-bg)] border border-[var(--warning-border)] rounded-lg text-xs text-[var(--warning-text)]">
              {totalIssues} compliance item{totalIssues > 1 ? 's' : ''} remain unaddressed.{' '}
              <Link href={`/proposals/${params.id}/compliance`} className="underline hover:no-underline">
                Review in Compliance Matrix
              </Link>{' '}
              before submitting.
            </div>
          )}
        ...
        {/* Export format */}
        <Card className="mb-6">
          <p className="text-[10px] font-medium text-[var(--text-tertiary)] tracking-wider uppercase mb-3">Select Export Format</p>
        ...
        <div className="flex items-center justify-between mb-6">
          <Button variant="default" icon={<ArrowLeft className="w-3.5 h-3.5" />} asChild>
            <Link href={`/proposals/${params.id}/workspace`}>Back to editing</Link>
          </Button>
          <Button variant="primary" icon={<Download className="w-3.5 h-3.5" />}>
            Download export
          </Button>
        </div>

        {/* Security note */}
        <p className="flex items-center justify-center gap-1.5 text-[10px] text-[var(--text-tertiary)]">
